...
async function renderInsights(){
  ...
  ct.innerHTML = `
    ...
    <div class="card animate-slide-up" style="animation-delay:.3s;overflow:hidden">
      <div class="section-header">
        <div class="section-icon-wrap" style="color:var(--color-accent-purple)">${insightIcons.ai}</div>
  ...
  // --- AI events ---
  ct.querySelectorAll('.ai-model-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      localStorage.setItem('medfocus_ai_model', btn.dataset.model);
      renderInsights();
    });
  });
  document.getElementById('btn-save-api-key')?.addEventListener('click', () => {
    const key = document.getElementById('ai-api-key').value.trim();
    const model = localStorage.getItem('medfocus_ai_model') || 'gemini';
    const storageKey = model === 'claude' ? 'medfocus_claude_key' : 'medfocus_gemini_key';
    if (key) { localStorage.setItem(storageKey, key); showToast(IC.check+' APIキーを保存しました'); }
    else { localStorage.removeItem(storageKey); showToast(IC.check+' APIキーを削除しました'); }
  });
  document.getElementById('btn-ai-export-week')?.addEventListener('click', () => exportForAI(allLogs, 'week'));
  document.getElementById('btn-ai-export-all')?.addEventListener('click', () => exportForAI(allLogs, 'all'));
  document.getElementById('btn-ai-ask')?.addEventListener('click', () => askAIAdvice(allLogs));
}


async function askAIAdvice(logs){
  const model=localStorage.getItem('medfocus_ai_model')||'gemini';
  const key=model==='claude'?localStorage.getItem('medfocus_claude_key'):localStorage.getItem('medfocus_gemini_key');
  if(!key){showToast(`⚠️ まず${model==='claude'?'Claude':'Gemini'} APIキーを設定してください`);return;}
  const resDiv=document.getElementById('ai-response');
  resDiv.style.display='block';resDiv.textContent=`🔄 ${model==='claude'?'Claude':'Gemini'}が分析中...`;
  const prompt=buildAIPrompt(logs);
  try{
    if(model==='claude'){
      ...
      if(data.content?.[0]?.text){resDiv.textContent=data.content[0].text;}
      else if(data.error){
        resDiv.textContent=formatAIError('Claude',data.error);
      }else{resDiv.textContent='❌ 予期しないレスポンス';}
    }else{
      ...
      const data=await res.json();
      if(data.candidates?.[0]?.content?.parts?.[0]?.text){resDiv.textContent=data.candidates[0].content.parts[0].text;}
      else if(data.error){
        resDiv.textContent=formatAIError('Gemini',data.error);
      }else{resDiv.textContent='❌ 予期しないレスポンス';}
    }
  }catch(e){resDiv.textContent='❌ 通信エラー: '+e.message;}
}

function formatAIError(provider,error){
  ...
  if(code===429||msg.includes('rate')||msg.includes('quota')||msg.includes('RESOURCE_EXHAUSTED')){
    ...
    const sec=retryMatch?retryMatch[1]:'数十';
    return`${provider}のレート制限に達しました。\n\n${sec}秒後に再度お試しください。\n\nヒント:\n• 無料枠には1日/1分あたりのリクエスト数制限があります\n• Claude Pro をお持ちなら「Claude」に切り替えてみてください\n• 「コピー」ボタンで手動でAIに貼り付けることもできます`;
  }
  if(code===401||code===403||msg.includes('API key')||msg.includes('authentication')){
    return`🔑 ${provider}のAPIキーが無効です。\n\n正しいキーを設定してください。`;
  }
  return`❌ ${provider}エラー (${code}): ${msg.slice(0,200)}`;
}

function buildAIPrompt(logs){
  const logicalToday=getLogicalDate(new Date());
  const monDiff=(logicalToday.getDay()+6)%7;
  const mon=new Date(logicalToday);mon.setDate(logicalToday.getDate()-monDiff);mon.setHours(5,0,0,0);
  const weekLogs=logs.filter(l=>new Date(l.started_at)>=mon);
  const totalMin=weekLogs.reduce((s,l)=>s+l.duration_minutes,0);
  const subMap={};weekLogs.forEach(l=>{const k=normalizeSubjectName(l.subject_name);subMap[k]=(subMap[k]||0)+l.duration_minutes;});
  const topSubs=Object.entries(subMap).sort((a,b)=>b[1]-a[1]);
  const fLogs=weekLogs.filter(l=>l.focus_level);
  const avgF=fLogs.length>0?(fLogs.reduce((s,l)=>s+Number(l.focus_level),0)/fLogs.length).toFixed(1):'N/A';
  const hF={};fLogs.forEach(l=>{const h=new Date(l.started_at).getHours();if(!hF[h])hF[h]={s:0,c:0};hF[h].s+=Number(l.focus_level);hF[h].c++;});
  const bestH=Object.entries(hF).map(([h,v])=>({h,avg:(v.s/v.c).toFixed(1)})).sort((a,b)=>b.avg-a.avg);
  const qbData=JSON.parse(localStorage.getItem('medfocus_qb_progress')||'{}');
  let qbStr='';
  Object.entries(qbData).forEach(([subId,rounds])=>{
    Object.entries(rounds).forEach(([rk,r])=>{
      if(r.total>0)qbStr+=`${subId} ${rk}周目: ${r.done}/${r.total}問完了, 正答${r.correct||0}問(${r.done>0?Math.round((r.correct||0)/r.done*100):0}%)\n`;
    });
  });
  return `あなたは医学部の国試対策アドバイザーです。以下の学習データを分析して、具体的で実践的なアドバイスを日本語で提供してください。

【今週の学習データ (${toLocalDateKey(mon)} 〜 ${toLocalDateKey(logicalToday)})】
総学習時間: ${formatMinutes(totalMin)}
平均集中度: ${avgF}/5.0
${bestH.length>0?`最も集中できる時間帯: ${bestH[0].h}時台(${bestH[0].avg})`:''}

■ 科目別学習時間:
${topSubs.map(([k,v])=>`- ${k}: ${formatMinutes(v)}`).join('\n')}
...
function exportForAI(logs,range){
  const logicalToday=getLogicalDate(new Date());
  let filtered=logs;
  let label='全期間';
  if(range==='week'){
    const monDiff=(logicalToday.getDay()+6)%7;
    const mon=new Date(logicalToday);mon.setDate(logicalToday.getDate()-monDiff);mon.setHours(5,0,0,0);
    filtered=logs.filter(l=>new Date(l.started_at)>=mon);
    label=`${toLocalDateKey(mon)} 〜 ${toLocalDateKey(logicalToday)}`;
  }
  const totalMin=filtered.reduce((s,l)=>s+l.duration_minutes,0);
  // Subject breakdown
  const subMap={};
  filtered.forEach(l=>{const k=normalizeSubjectName(l.subject_name);subMap[k]=(subMap[k]||0)+l.duration_minutes;});
  const topSubs=Object.entries(subMap).sort((a,b)=>b[1]-a[1]);
  // Focus
  const fLogs=filtered.filter(l=>l.focus_level);
  const avgF=fLogs.length>0?(fLogs.reduce((s,l)=>s+Number(l.focus_level),0)/fLogs.length).toFixed(1):'N/A';
  // By hour
  const hF={};
  fLogs.forEach(l=>{const h=new Date(l.started_at).getHours();if(!hF[h])hF[h]={s:0,c:0};hF[h].s+=Number(l.focus_level);hF[h].c++;});
  const bestH=Object.entries(hF).map(([h,v])=>({h,avg:(v.s/v.c).toFixed(1)})).sort((a,b)=>b.avg-a.avg);
  // Streak
  const studyDateSet=new Set(logs.map(l=>toLocalDateKey(getLogicalDate(new Date(l.started_at)))));
  let streak=0;const dI=new Date(logicalToday);
  while(studyDateSet.has(toLocalDateKey(dI))){streak++;dI.setDate(dI.getDate()-1);}
  // QB progress
  const qbData=JSON.parse(localStorage.getItem('medfocus_qb_progress')||'{}');
  const volTotals={};
  Object.entries(qbData).forEach(([subId,rounds])=>{
    const vol=subId.startsWith('1')?'vol.1':subId.startsWith('2')?'vol.2':subId.startsWith('3')?'vol.3':'other';
    if(!volTotals[vol])volTotals[vol]={done:0,total:0};
    Object.values(rounds).forEach(r=>{volTotals[vol].done+=r.done||0;volTotals[vol].total+=r.total||0;});
  });

  let text=`【MedFocus 学習レポート】\n期間: ${label}\n総学習時間: ${formatMinutes(totalMin)}\n連続記録: ${streak}日\n\n■ 科目別学習時間\n`;
  topSubs.forEach(([k,v],i)=>{text+=`${i+1}. ${k}: ${formatMinutes(v)}\n`;});
  text+=`\n■ 集中度分析\n平均集中度: ${avgF}/5.0\n`;
  if(bestH.length>0)text+=`最高集中時間帯: ${bestH[0].h}時台 (${bestH[0].avg})\n`;
  if(Object.keys(volTotals).length>0){
    text+=`\n■ QB進捗\n`;
    Object.entries(volTotals).forEach(([vol,v])=>{
      text+=`${vol}: ${v.done}/${v.total} (${v.total>0?Math.round(v.done/v.total*100):0}%)\n`;
    });
  }
  text+=`\nこのデータを元に、学習計画の改善アドバイスをお願いします。`;
  navigator.clipboard.writeText(text).then(()=>showToast(' クリップボードにコピーしました！AIに貼り付けてください')).catch(()=>showToast(' コピーに失敗しました'));
}
