...
eq('vol.4 は多肢選択問題と4連問の2部構成',
   [W.qbSectionOf('4M2C').name, W.qbSectionOf('4R2C').name],
...
eq('多肢選択問題は 1A から 3D まで並ぶ（先頭と末尾）',
   ...
   [{ id: '4M1A', name: '多肢選択 1A 細胞生物学' },
    { id: '4M3D', name: '多肢選択 3D 公衆衛生' }]);
eq('4連問は 1A から 3D まで並ぶ（先頭と末尾）',
   [linked[0], linked[BASE_COUNT - 1]],
   [{ id: '4R1A', name: '4連問 1A 細胞生物学' },
    { id: '4R3D', name: '4連問 3D 公衆衛生' }]);
eq('科目IDは重複しない',
   new Set(multi.concat(linked).map(s => s.id)).size, BASE_COUNT * 2);

// ---------- 科目の解決 ----------
eq('科目IDからセクションを引ける', W.qbSectionOf('4R2C').short, '4連問');
eq('表示名からもセクションを引ける', W.qbSectionOf('4連問 2C 循環器').short, '4連問');
eq('多肢選択も同様', W.qbSectionOf('4M2C').short, '多肢選択');
...
eq('vol.4 の科目IDは元の科目へ畳める', W.baseSubjectIdOf('4R2C'), '2C');
...
// ---------- 表示名は畳まない / 集計名は畳む ----------
eq('学習ログの表示は vol.4 のまま', W.normalizeSubjectName('4R2C'), '4連問 2C 循環器');
eq('多肢選択の表示名', W.normalizeSubjectName('4M2C'), '多肢選択 2C 循環器');
eq('表示名から科目IDへ戻せる（編集で保存し直しても壊れない）',
   W.subjectIdOfName('4連問 2C 循環器'), '4R2C');

eq('時間の集計は元の科目へ寄せる（4連問）', W.studySubjectName('4R2C'), '2C 循環器');
...
// ---------- 講義動画を持たない科目 ----------
ok('4連問は講義動画を持たない', W.isQbOnlySubject('4R2C') === true);
ok('多肢選択は講義動画を持たない', W.isQbOnlySubject('4M1A') === true);
ok('2C は講義動画を持つ', W.isQbOnlySubject('2C') === false);
eq('vol.4 のログでも版は元の科目で解く（基礎医学はCBT版）',
   W.logVideoEdition({ subject_name: '4M1A' }), 'cbt');
eq('vol.4 のログでも版は元の科目で解く（臨床は国試版）',
   W.logVideoEdition({ subject_name: '4R2C' }), 'kokushi');

// ---------- カテゴリ / 色 ----------
eq('4連問は vol.4 4連問のカテゴリ', W.subjectCategoryOf('4R2C'), 'cat-vol4-linked');
eq('多肢選択は vol.4 多肢選択問題のカテゴリ', W.subjectCategoryOf('4M2C'), 'cat-vol4-multi');
eq('元の科目のカテゴリは変わらない', W.subjectCategoryOf('2C'), 'cat-vol2');
ok('vol.4 は元の科目と違う色を持つ',
   W.subjectColorOf('4R2C') !== W.subjectColorOf('2C'));
...
(function qbProgressGoesToSection() {
  ...
  window.localStorage.setItem('medfocus_qb_progress', JSON.stringify({
    ...
    '4R2C': { '1': { done: 0, total: 40,  correct: 0 } }
  }));
  const res = W.applyQbSessionToProgress('4R2C', 20, 16);
  const qb = W.getQBProgress();
  eq('4連問の1周目が進む', qb['4R2C']['1'], { done: 20, total: 40, correct: 16 });
...
(function qbProgressCarriesOverWithinSection() {
  window.localStorage.setItem('medfocus_qb_progress', JSON.stringify({
    '4M3D': { '1': { done: 8, total: 10, correct: 6 } }
  }));
  W.applyQbSessionToProgress('4M3D', 5, 5);
  const qb = W.getQBProgress();
  eq('1周目を埋めきる', qb['4M3D']['1'], { done: 10, total: 10, correct: 8 });
  eq('あふれた分は2周目へ繰り越す', qb['4M3D']['2'], { done: 3, total: 10, correct: 3 });
})();

// ---------- 逆算プラン ----------
(function planMatching() {
  const linkedLog = { subject_name: '4R2C', questions_solved: 10 };
  ...
  ok('4連問のログは 4連問 2C の問題数プランに入る',
     W.planLogMatches({ subject_id: '4R2C', unit: 'q' }, linkedLog) === true);
  ok('2C のログは 4連問 2C のプランに入らない',
     W.planLogMatches({ subject_id: '4R2C', unit: 'q' }, baseLog) === false);
  ok('2C のログは今までどおり 2C のプランに入る',
     W.planLogMatches({ subject_id: '2C', unit: 'q' }, baseLog) === true);
  ok('表示名で保存されたログでも同じ結論になる',
     W.planLogMatches({ subject_id: '4R2C', unit: 'q' },
                      { subject_name: '4連問 2C 循環器' }) === true);
})();

// ---------- vol.4 の vol 集計 ----------
(function volAggregate() {
  const cat = { id: 'cat-vol4-linked', subjects: [{ id: '4R2C' }, { id: '4R2J' }] };
  const agg = W.volRoundAggregate(
    { '4R2C': { '1': { done: 20, total: 40, correct: 16 } },
      '4R2J': { '1': { done: 10, total: 60, correct: 5  } } },
    {}, cat);
...
(function strayVideoRecordStaysVisible() {
  ...
  ok('vol.4 に動画の記録は普通は無い', W.hasVideoRecordFor('4R2C') === false);
  window.localStorage.setItem('medfocus_video_progress', JSON.stringify({
    '4R2C': { kokushi: { done: 2, total: 5 } }
  }));
  ok('間違って入った動画の記録は見つけられる', W.hasVideoRecordFor('4R2C') === true);
  ok('記録があれば編集できる行を出す',
     W.videoTrackerBlockHtml('4R2C').indexOf('vid-ed-row') >= 0);
  window.localStorage.removeItem('medfocus_video_progress');
})();
