<!DOCTYPE html>
<html lang="ja">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>MedFocus - 医学部生のための学習管理プラットフォーム</title>
  <meta name="description" content="医学部生専用のクローズド学習管理・SNSアプリ。仲間と学習進捗を共有し、共に成長しよう。" />
  <link rel="preconnect" href="https://fonts.googleapis.com" />
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin />
  <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&family=Noto+Sans+JP:wght@300;400;500;600;700&display=swap" rel="stylesheet" />
  <script src="https://cdn.jsdelivr.net/npm/chart.js@4.4.0/dist/chart.umd.min.js"></script>
  <script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"></script>
  <script src="https://unpkg.com/lucide@latest"></script>
  <style>
    :root { color-scheme: light dark; }
    #app { min-height: 100vh; }
    svg { width: 24px; height: 24px; display: inline-block; vertical-align: middle; }
  </style>
  <link rel="icon" type="image/png" href="icon-192.png" />
  <link rel="apple-touch-icon" href="apple-touch-icon.png">
  <link rel="manifest" href="manifest.json">
  <meta name="theme-color" content="#4ecdc4">
  <script type="module" crossorigin src="./assets/index-dgjPl2Xr.js"></script>
  <link rel="stylesheet" crossorigin href="./assets/index-C_cLDAuc.css">
</head>
<body>
  <div id="app">
    <aside id="sidebar"></aside>
    <main id="main-content">
      <div id="page-container"></div>
    </main>
  </div>
  <script>
    if ('serviceWorker' in navigator) {
      navigator.serviceWorker.getRegistrations().then(registrations => {
        for(let registration of registrations) {
          registration.unregister();
          console.log('DEBUG: Service Worker unregistered');
        }
      });
    }
  </script>
</body>
</html>
